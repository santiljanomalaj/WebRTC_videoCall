...
function myjsapp(peerClient) {
    ...
    function EventListeners() {
        ...
        $(document).on('click', '.videoCall', function() {
            ...
            peerClient.makeCall(this.id, isVideoCall);
            return false
        });
    ...
    // Show Username Modal
    var basicURL=(location.href).split('#')
    var username = cookie.get('username');

    if(username != basicURL[1]) {
        $('#user-name').val(basicURL[1])
        startPeerClient(basicURL[1])
    ...
    return {
        ...
        createChatWindow: function(id) {
            var toPeerId = id;
            var panel = $('<div class="panel panel-primary chat-div"><div class="panel-heading"><h2>'+toPeerId+'</h2></div>' +
            ...
            // $(".video-call").html(videoCall)
            // $(".audio-call").html(audioCall)
        ...
        updateOnlieUsers : function (users) {
            ...
            for (var i = 0; i < users.length; i++) {
                var usr = '<li class="peeruser" style="padding:3px;font-size: 20px;color: #0f409a;" type="1"><div style="float:left;">'+ users[i] + '</div><div style="float:right;"><button style="margin-right:10px;" id="'+users[i]+'" class="btn btn-success message">Message</button><button class="btn btn-primary videoCall" id="'+users[i]+'" style="margin-right:10px;">Call</button></div></li>'
                list.append(usr);
            }
        }
    };
